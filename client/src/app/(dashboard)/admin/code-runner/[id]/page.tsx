...
import { useGetProblemByIdQuery, useUpdateProblemMutation } from '@/state/api/codeRunnerApi';
...
export default function EditProblemPage({ params }: { params: { id: string } }) {
  ...
  const { data: problemData, isLoading } = useGetProblemByIdQuery(params.id);
  ...
  useEffect(() => {
    if (problemData?.data) {
      const { _id, ...rest } = problemData.data;
      setProblem(rest);
    }
  }, [problemData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateProblem({ id: params.id, problem }).unwrap();
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-white">Edit Problem</h1>
      </div>
