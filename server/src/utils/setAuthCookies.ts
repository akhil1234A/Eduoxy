...
export const setAuthCookies = (res: Response, tokens: { accessToken: string; refreshToken: string }, user: { id: string; userType: string, userName: string }) => {
  ...
  res.cookie("userId", user.id, { ...cookieOptions, httpOnly: false, maxAge: 7 * 24 * 60 * 60 * 1000 });
  res.cookie("userType", user.userType, { ...cookieOptions, httpOnly: false, maxAge: 7 * 24 * 60 * 60 * 1000 });
  res.cookie("userName", user.userName, { ...cookieOptions, httpOnly: false, maxAge: 7 * 24 * 60 * 60 * 1000 });
};